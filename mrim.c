 ...
 *             USER AGENT
 ******************************************/
static gboolean ua_matches(const gchar *string, const gchar *pattern, ua_data *result)
{
...
void blist_send_sms(PurpleConnection *gc, PurpleRequestFields *fields)
{
	...
	mrim_send_sms(list->data, message, (mrim_data*)(gc->proto_data));
}
...
static void mrim_set_status(PurpleAccount *acct, PurpleStatus *status)
{
	...
	const char *prpl_status_id = purple_status_get_id(status);
